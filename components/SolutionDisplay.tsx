...
const SolutionDisplay: React.FC<SolutionDisplayProps> = ({ content, isThinking, language }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Professor Neural Resolution',
          text: content,
    ...
    <div className="bg-white dark:bg-slate-900 rounded-5xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 transition-all duration-500 transform animate-in zoom-in-95 duration-700">
      <div className="bg-brand-600 dark:bg-brand-800 px-8 py-5 flex justify-between items-center text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="font-extrabold uppercase tracking-[0.2em] text-[9px] opacity-70">{t.respostaDireta}</span>
            <span className="font-bold text-base leading-tight">{t.resultadoNeural}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleRead} disabled={isReading} className={`p-2.5 rounded-xl border border-white/10 ${isReading ? 'bg-white text-brand-600' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
            <Volume2 className={`w-4 h-4 ${isReading ? 'animate-pulse' : ''}`} />
          </button>
          <button onClick={handleCopy} className={`p-2.5 rounded-xl border border-white/10 ${isCopied ? 'bg-white text-brand-600' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t.otimizado}</span>
        </div>
        <button onClick={handleShare} className="group flex items-center gap-2">
            <span className="text-[10px] font-black text-slate-400 group-hover:text-brand-500 uppercase tracking-widest transition-colors">{t.compartilhar}</span>
